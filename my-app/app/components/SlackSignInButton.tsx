...
/**
 * SlackSignInButton
 * The primary CTA on the login screen.
 * Renders the Slack-purple button described in the design doc.
 */
export default function SlackSignInButton({ onClick }: SlackSignInButtonProps) {
  return (
    <button
      onClick={onClick}
      className="w-full flex items-center justify-center gap-3 bg-[#4a154b] hover:bg-[#3d1040] active:scale-95 transition-all text-white font-semibold py-3 px-6 rounded-xl text-sm shadow-md hover:shadow-lg"
    >
      {/* Slack "S" icon — inline SVG, no external asset */}
      <svg
        width="18"
        height="18"
